...
export function SettingsProvider({ children }) {
  ...
  async function updateSetting(key, value) {
    ...
    setSettings(s => ({ ...s, [key]: strVal }))
    await supabase.from('site_settings').upsert({ key, value: strVal }).eq('key', key)
  }
  ...
  async function updateSettings(updates) {
    ...
    for (const [key, value] of Object.entries(updates)) {
      ...
      await supabase.from('site_settings').update({ value: strVal }).eq('key', key)
    }
  ...
  function getJSON(key, fallback = []) {
    try {
      return JSON.parse(settings[key] ?? JSON.stringify(fallback))
    } catch {
      return fallback
    }
  }
  ...
  return (
    <SettingsContext.Provider value={{ settings, loading, get, getJSON, updateSetting, updateSettings, fetchSettings }}>
      {children}
